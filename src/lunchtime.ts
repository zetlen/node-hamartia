export default function getNextLunchtime (hours: number, minutes: number, DateCtr: typeof Date): Date {
	...
	// tomorrow's lunch
	if (lunchtime.getTime() < DateCtr.now()) lunchtime.setDate(lunchtime.getDate() + 1);
